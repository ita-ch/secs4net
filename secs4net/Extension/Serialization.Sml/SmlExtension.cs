...
namespace Secs4Net.Sml
{
	public static class SmlExtension
	{
		...
		public static SecsMessage ToSecsMessage(this TextReader sr)
		{
			...
			var indexS = line.IndexOf("'S") + 2;
			var indexF = line.IndexOf("F") + 1;
			var lengthS = indexF - 1 - indexS;
			var byteS = byte.Parse(line.Substring(indexS, lengthS));

			var lengthF = line.IndexOf('\'', 2) - indexF;
			var byteF = byte.Parse(line.Substring(indexF, lengthF));

			var indexOfW = line.IndexOf(" W ");
			var replyExpected = indexOfW != -1;
			string name;
			if (replyExpected)
				name = line.Substring(indexOfW + 3);
			else
				name = line.Substring(indexF + lengthF + 3);
			...
			return new SecsMessage(byteS, byteF, name, rootItem, replyExpected);
		}
